...
7. **Swappable methodologies.** Document-level scorers implement `BaseHawkDoveScorer` and are selected via a registry (`heuristic`, `roberta`). RoBERTa (`gtfintechlab/fomc-hawkish-dovish`) is the ML baseline for A/B comparison against the lexicon heuristic. Component scores from RoBERTa are keyword-routed over labeled sentences. Claude/Anthropic is not used for hawk–dove scoring.
